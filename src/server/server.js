...
web3.eth.getAccounts((_, accounts) => {
    ...
    flightSuretyData.methods.authorizeCaller(config.appAddress).send({ from: owner }, (error, _) => {
        if (error) {
            console.log(error);
        } else {
    ...
    for (let a = ORACLES_ACCOUNT_OFFSET; a < ORACLES_COUNT + ORACLES_ACCOUNT_OFFSET; a++) {
        flightSuretyApp.methods.registerOracle().send({ from: accounts[a], value: web3.utils.toWei("1", 'ether'), gas: 4500000 }, (error, _) => {
            if (error) {
                console.log(error);
            } else {
    ...
    if (error) {
        ...
        for (let a = 0; a < oracles.length; a++) {
            if (oracles[a].index.includes(index)) {
                flightSuretyApp.methods.submitOracleResponse(index, airline, flight, timestamp, statusCode).send({ from: oracles[a].address }, (error, _) => {
                    if (error) {
                        console.log(error);
                    } else {
